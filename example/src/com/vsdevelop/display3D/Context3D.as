package com.vsdevelop.display3D
{
	...
	import com.vsdevelop.display3D.textures.CubeTexture;
	import com.vsdevelop.display3D.textures.RectangleTexture;
	...
	public class Context3D
	{
		...
		{
			Glfw.glfwSwapBuffers(_stage3d.activeWindow);
		}
		 ...
		 * 创建2D纹理
		 */
		public function createTexture(width:int, height:int, format:String = "bgra", optimizeForRenderToTexture:Boolean = false, streamingLevels:int = 0):Texture
		{
			// 添加参数验证
			if (width <= 0 || height <= 0)
			{
			...
			return new Texture(this, width, height, format, optimizeForRenderToTexture);
		}
		
		/**
		 * 创建立方体贴图
		 */
		public function createCubeTexture(size:int, format:String = "bgra", optimizeForRenderToTexture:Boolean = false, streamingLevels:int = 0):CubeTexture
		{
			// 添加参数验证
			if (size <= 0)
			{
			...
			return new CubeTexture(this, size, format);
		}
		
		/**
		 * 创建矩形纹理
		 */
		public function createRectangleTexture(width:int, height:int, format:String = "bgra", optimizeForRenderToTexture:Boolean = false):RectangleTexture
		{
			// 添加参数验证
			if (width <= 0 || height <= 0)
			{
			...
			return new RectangleTexture(this, width, height, format);
		}
		
		/**
		 * 设置纹理到指定的纹理单元
		 */
		public function setTextureAt(sampler:int, texture:Texture):void
		{
			// 添加参数验证
			if (sampler < 0)
			{
				throw new Error("Context3D.setTextureAt: sampler index cannot be negative");
			}
